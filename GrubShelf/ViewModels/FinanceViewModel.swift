...
final class FinanceViewModel {
    ...
    /// Trip spend aggregated by calendar day (start-of-day keys) for the current loaded period.
    var spendByStartOfDay: [Date: Int] {
        let calendar = Calendar.current
        var dict: [Date: Int] = [:]
        for trip in trips {
            guard let cost = trip.totalCostMinor, cost > 0 else { continue }
            let day = calendar.startOfDay(for: trip.date)
            dict[day, default: 0] += cost
        }
        return dict
    }

    /// Normalized daily cells for the calendar heatmap (matches budget period: week or month grid).
    var heatmapDayCells: [(date: Date, amountMinor: Int, intensity: Double)] {
        guard let settings = financeSettings else { return [] }
        let days = ExpenseHeatmapBuilder.daysForBudgetPeriod(
            budgetPeriod: settings.budgetPeriod,
            reference: Date.now
        )
        return ExpenseHeatmapBuilder.normalizedDays(
            calendarDays: days,
            spendByStartOfDay: spendByStartOfDay
        )
    }

    /// Snapshots ordered for chart X-axis (string sort works for `YYYY-MM` and `YYYY-W%02d` week numbers 01–53).
    var sortedHistoricalSnapshots: [PeriodSnapshot] {
        historicalSnapshots.sorted { $0.period < $1.period }
    }
    ...
    /// Short label for dashboard budget card (follows `budgetPeriod` when set).
    var budgetPeriodKindLabel: String {
        switch financeSettings?.budgetPeriod {
        case .weekly: return String(localized: "Weekly budget")
        case .monthly: return String(localized: "Monthly budget")
        case .none: return String(localized: "Monthly budget")
        }
    }
    ...
    func logManualPurchase(amountMinor: Int, date: Date, storeName: String? = nil) async throws {
        ...
        ToastManager.shared.show("Logged. Nice.", style: .success)
