...
private let logger = Logger(subsystem: "com.grubshelf", category: "Network")
    ...
    for attempt in 1...maxAttempts {
        do {
            ...
            if attempt < maxAttempts {
                ...
                logger.debug("Retry \(attempt)/\(maxAttempts) after \(error.localizedDescription)")
                try? await Task.sleep(for: delay)
            }
        }
    }

    throw lastError ?? NSError(domain: "NetworkRetry", code: -1, userInfo: [NSLocalizedDescriptionKey: "All retry attempts failed"])
}
